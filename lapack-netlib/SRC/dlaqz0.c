...
/* Subroutine */ void dlaqz0_(char *wants, char *wantq, char *wantz, integer *
	n, integer *ilo, integer *ihi, doublereal *a, integer *lda, 
	doublereal *b, integer *ldb, doublereal *alphar, doublereal *alphai, 
	doublereal *beta, doublereal *q, integer *ldq, doublereal *z__, 
	integer *ldz, doublereal *work, integer *lwork, integer *rec, integer 
	*info)
{
    ...
    extern /* Subroutine */ void dlaqz3_(logical *, logical *, logical *, 
    ...
    extern /* Subroutine */ void dlaqz4_(logical *, logical *, logical *, 
...
/*     Get the parameters */
    ...
    *(unsigned char *)&jbcmpz[2] = *(unsigned char *)wantz;
    nmin = ilaenv_(&c__12, "DLAQZ0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6, (
	    ftnlen)3);
    nwr = ilaenv_(&c__13, "DLAQZ0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6, (
	    ftnlen)3);
    nwr = f2cmax(2,nwr);
/* Computing MIN */
    i__1 = *ihi - *ilo + 1, i__2 = (*n - 1) / 3, i__1 = f2cmin(i__1,i__2);
    nwr = f2cmin(i__1,nwr);
    nibble = ilaenv_(&c__14, "DLAQZ0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6, 
	    (ftnlen)3);
    nsr = ilaenv_(&c__15, "DLAQZ0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6, (
	    ftnlen)3);
/* Computing MIN */
...
/* Computing MAX */
    ...
    rcost = ilaenv_(&c__17, "DLAQZ0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6, (
	    ftnlen)3);
    ...
    nbr = nsr + itemp1;
    if (*n < nmin || *rec >= 2) {
	dhgeqz_(wants, wantq, wantz, n, ilo, ihi, &a[a_offset], lda, &b[
...
/*     Workspace query to dlaqz3 */
    nw = f2cmax(nwr,nmin);
    dlaqz3_(&ilschur, &ilq, &ilz, n, ilo, ihi, &nw, &a[a_offset], lda, &b[
...
/*     Workspace query to dlaqz4 */
    dlaqz4_(&ilschur, &ilq, &ilz, n, ilo, ihi, &nsr, &nbr, &alphar[1], &
...
/* Computing MAX */
	d__4 = smlnum, d__5 = ulp * ((d__1 = a[istop - 1 + (istop - 1) * 
		a_dim1], abs(d__1)) + (d__2 = a[istop - 2 + (istop - 2) * 
		a_dim1], abs(d__2)));
	if ((d__3 = a[istop - 1 + (istop - 2) * a_dim1], abs(d__3)) <= f2cmax(
		d__4,d__5)) {
...
/* Computing MAX */
	    d__4 = smlnum, d__5 = ulp * ((d__1 = a[istop + istop * a_dim1], 
		    abs(d__1)) + (d__2 = a[istop - 1 + (istop - 1) * a_dim1], 
		    abs(d__2)));
	    if ((d__3 = a[istop + (istop - 1) * a_dim1], abs(d__3)) <= f2cmax(
		    d__4,d__5)) {
...
/* Computing MAX */
	d__4 = smlnum, d__5 = ulp * ((d__1 = a[istart + 1 + (istart + 1) * 
		a_dim1], abs(d__1)) + (d__2 = a[istart + 2 + (istart + 2) * 
		a_dim1], abs(d__2)));
	if ((d__3 = a[istart + 2 + (istart + 1) * a_dim1], abs(d__3)) <= f2cmax(
		d__4,d__5)) {
...
/* Computing MAX */
	    d__4 = smlnum, d__5 = ulp * ((d__1 = a[istart + istart * a_dim1], 
		    abs(d__1)) + (d__2 = a[istart + 1 + (istart + 1) * a_dim1]
		    , abs(d__2)));
	    if ((d__3 = a[istart + 1 + istart * a_dim1], abs(d__3)) <= f2cmax(
		    d__4,d__5)) {
...
/* Computing MAX */
	    d__4 = smlnum, d__5 = ulp * ((d__1 = a[k + k * a_dim1], abs(d__1))
		     + (d__2 = a[k - 1 + (k - 1) * a_dim1], abs(d__2)));
	    if ((d__3 = a[k + (k - 1) * a_dim1], abs(d__3)) <= f2cmax(d__4,d__5))
		     {
		a[k + (k - 1) * a_dim1] = 0.;
		istart2 = k;
		myexit_();
...
/*        slow down the method when many infinite eigenvalues are present */
	k = istop;
	while(k >= istart2) {
	    if ((d__1 = b[k + k * b_dim1], abs(d__1)) < btol) {
/*              A diagonal element of B is negligible, move it */
/*              to the top and deflate it */
		i__2 = istart2 + 1;
		for (k2 = k; k2 >= i__2; --k2) {
		    dlartg_(&b[k2 - 1 + k2 * b_dim1], &b[k2 - 1 + (k2 - 1) * 
			    b_dim1], &c1, &s1, &temp);
		    b[k2 - 1 + k2 * b_dim1] = temp;
		    b[k2 - 1 + (k2 - 1) * b_dim1] = 0.;
		    i__3 = k2 - 2 - istartm + 1;
		    drot_(&i__3, &b[istartm + k2 * b_dim1], &c__1, &b[istartm 
			    + (k2 - 1) * b_dim1], &c__1, &c1, &s1);
/* Computing MIN */
		    i__4 = k2 + 1;
		    i__3 = f2cmin(i__4,istop) - istartm + 1;
		    drot_(&i__3, &a[istartm + k2 * a_dim1], &c__1, &a[istartm 
			    + (k2 - 1) * a_dim1], &c__1, &c1, &s1);
		    if (ilz) {
			drot_(n, &z__[k2 * z_dim1 + 1], &c__1, &z__[(k2 - 1) *
				 z_dim1 + 1], &c__1, &c1, &s1);
		    }
		    if (k2 < istop) {
			dlartg_(&a[k2 + (k2 - 1) * a_dim1], &a[k2 + 1 + (k2 - 
				1) * a_dim1], &c1, &s1, &temp);
			a[k2 + (k2 - 1) * a_dim1] = temp;
			a[k2 + 1 + (k2 - 1) * a_dim1] = 0.;
			i__3 = istopm - k2 + 1;
			drot_(&i__3, &a[k2 + k2 * a_dim1], lda, &a[k2 + 1 + 
				k2 * a_dim1], lda, &c1, &s1);
			i__3 = istopm - k2 + 1;
			drot_(&i__3, &b[k2 + k2 * b_dim1], ldb, &b[k2 + 1 + 
				k2 * b_dim1], ldb, &c1, &s1);
			if (ilq) {
			    drot_(n, &q[k2 * q_dim1 + 1], &c__1, &q[(k2 + 1) *
				     q_dim1 + 1], &c__1, &c1, &s1);
			}
		    }
		}
		if (istart2 < istop) {
		    dlartg_(&a[istart2 + istart2 * a_dim1], &a[istart2 + 1 + 
			    istart2 * a_dim1], &c1, &s1, &temp);
		    a[istart2 + istart2 * a_dim1] = temp;
		    a[istart2 + 1 + istart2 * a_dim1] = 0.;
		    i__2 = istopm - (istart2 + 1) + 1;
		    drot_(&i__2, &a[istart2 + (istart2 + 1) * a_dim1], lda, &
			    a[istart2 + 1 + (istart2 + 1) * a_dim1], lda, &c1,
			     &s1);
		    i__2 = istopm - (istart2 + 1) + 1;
		    drot_(&i__2, &b[istart2 + (istart2 + 1) * b_dim1], ldb, &
			    b[istart2 + 1 + (istart2 + 1) * b_dim1], ldb, &c1,
			     &s1);
		    if (ilq) {
			drot_(n, &q[istart2 * q_dim1 + 1], &c__1, &q[(istart2 
...
/* Computing 2nd power */
	...
	dlaqz3_(&ilschur, &ilq, &ilz, n, &istart2, &istop, &nw, &a[a_offset], 
...
/*           Exceptional shift.  Chosen for no particularly good reason. */
	    ...
	    if ((doublereal) maxit * safmin * (d__1 = a[istop + (istop - 1) * 
		    a_dim1], abs(d__1)) < (d__2 = a[istop - 1 + (istop - 1) * 
		    a_dim1], abs(d__2))) {
...
/* Computing 2nd power */
	i__5 = nblock;
	i__4 = *lwork - (i__5 * i__5 << 1);
	dlaqz4_(&ilschur, &ilq, &ilz, n, &istart2, &istop, &ns, &nblock, &
...
L80:
    dhgeqz_(wants, wantq, wantz, n, ilo, ihi, &a[a_offset], lda, &b[b_offset],
